...
extern float rot;
extern bool EarthBool, MarsBool;
 ...
 start(START)
{
	...
	open = start + chrono::milliseconds(5000);
	increment = 0;
	increment2 = 1;
...
void Portal::openPortal()
{
	...
	if(!EarthBool && !MarsBool && (now < stopPortal))
	{
		pos.x+=look.x*5;
		pos.y+=look.y*5;
		pos.z+=look.z*5;
	}
	else if(now < stopPortal)
	{
		pos.x+=look.x;
		pos.y+=look.y;
		pos.z+=look.z;
